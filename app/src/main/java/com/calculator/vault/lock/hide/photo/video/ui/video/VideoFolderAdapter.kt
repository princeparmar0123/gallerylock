...
import com.calculator.vault.lock.hide.photo.video.ui.video.VideoFolderActivity.Companion.select_video_datas
import java.io.File


class VideoFolderAdapter(var context: Activity) : BaseAdapter<ItemVideoBinding, Media_Data>(R.layout.item_video) {
...
//            }
    ...
    fun addRemoveSelectionList(holder: ItemVideoBinding, position: Int,item: Media_Data) {
        try {
            ...
            VideoFolderActivity.selecter_fvideo?.isVisible = true
            if (select_video_datas.contains(item)) {
                select_video_datas.remove(item)
                holder.selected.visibility = View.GONE
            } else {
                select_video_datas.add(item)
                holder.selected.visibility = View.VISIBLE
            }
            if (select_video_datas.size == 0) {
                IsLongClick = false
                notifyItemChanged(position)
                VideoFolderActivity.IsSelectAll = false
                VideoFolderActivity.selecter_fvideo?.isVisible = false
                VideoFolderActivity.selecter_fvideo?.icon = context.resources.getDrawable(R.drawable.iv_unselect)
            }
            if (select_video_datas.size == displayList.size) {
                VideoFolderActivity.IsSelectAll = true
                VideoFolderActivity.selecter_fvideo?.icon = context.resources.getDrawable(R.drawable.iv_select)
            } else {
                VideoFolderActivity.IsSelectAll = false
                VideoFolderActivity.selecter_fvideo?.icon = context.resources.getDrawable(R.drawable.iv_unselect)
            }
            VideoFolderActivity.video_hide?.text = "Hide (" + select_video_datas.size.toString() + ")"
